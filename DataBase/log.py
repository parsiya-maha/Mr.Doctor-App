...
# Give true and false predict values 
def true_false_value():
    ...
    data = jmespath.search(f"[? id==`{_id}`].point",reader)

    return data.count(1),data.count(0)


def test_true_false_value():
    with open(path_log_data) as F:
        reader = json.load(F)
    ...
    return jmespath.search(f"[? id==`{_id}`].point",reader)
...
def clear_all_log():
    ...
    res = jmespath.search(f"[? id!=`{_id}`]",reader)
